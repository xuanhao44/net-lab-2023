...
5. 去掉 IP 报头。注意不要使用野指针。

6. 调用 net_in() 函数向上层传递数据包。照着指导书做就行。
...
2. 必做任务只要求做到 UDP，TCP 不需要做，所以在做 IP/ICMP 自测时，当收到 TCP 报文可以当作不能处理，需回送一个 ICMP 协议不可达报文。但是当你已经做到 UDP/TCP 以上协议，用另外一套自测环境，就不用再返回来做 IP/ICMP 自测。
...
上面这张图中，我特意调整成了 4 字节一行，和图示的结构一样。但我自己心里明白这些数据是怎么排列的，调整成不管是 8 字节一行，16字节一行都行。
