...
typedef struct ip_hdr
{
    ...
    uint8_t src_ip[NET_IP_LEN]; // 源 IP
    uint8_t dst_ip[NET_IP_LEN]; // 目标 IP
} ip_hdr_t;
#pragma pack()

#define IP_HDR_LEN_PER_BYTE 4      // ip 包头长度单位
#define IP_HDR_OFFSET_PER_BYTE 8   // ip 分片偏移长度单位
#define IP_VERSION_4 4             // ipv4
#define IP_MORE_FRAGMENT (1 << 13) // ip 分片 mf 位
